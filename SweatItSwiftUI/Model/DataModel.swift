...
class Workout {
    ...
    let exampleWorkoutList: Array<Workout_t> = [
        .init(workoutName: "Upper Body", workoutDescription: "Build a wider looking shoulders and a fuller chest with this workout. And a stronger one at best too.", workoutCategory: .strength, workoutImage: "upperbody", exercises: [Exercise.current.chestDip, Exercise.current.inclinePushUp, Exercise.current.pushUp, Exercise.current.benchPress], timing: Calendar.current.date(byAdding: .hour, value: -2 ,to: .now)!),
        .init(workoutName: "Lower Body", workoutDescription: "Got chicken legs? No problem! With this workout you'll get more dates for sure 😏. Get your guts together, shit is about to get real 🗿.", workoutCategory: .strength, workoutImage: "lowerbody", exercises: [Exercise.current.bodyweightSquat, Exercise.current.calfRaise, Exercise.current.gluteBridge, Exercise.current.walkingLunge], timing: Calendar.current.date(byAdding: .hour, value: -5 ,to: .now)!),
        .init(workoutName: "Arm Builder", workoutDescription: "Got No guns? What a looser 😒. Get guns now with this workout or you are gay🤡.", workoutCategory: .strength, workoutImage: "arms", exercises: [Exercise.current.closeGripPushups, Exercise.current.bicepCurl, Exercise.current.tricepDips], timing: Calendar.current.date(byAdding: .hour, value: -4, to: .now)!),
        .init(workoutName: "Core Create", workoutDescription: "Bro you got family pack instead of 6? Fear not! Be beach ready in only 30 mins a day. Let the girls know you've entered the beach premisis 😉.", workoutCategory: .strength, workoutImage: "core", exercises: [Exercise.current.plank, Exercise.current.bicycleCrunch, Exercise.current.mountainClimbers, Exercise.current.mountainClimbers], timing: Calendar.current.date(byAdding: .hour, value: -10, to: .now)!)
    ]
